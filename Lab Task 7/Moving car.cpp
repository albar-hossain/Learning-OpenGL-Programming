...
float _angle1 = 0.0f;
float _carPositionX = -4.0f; // Initial position of the car
...
void drawScene() {
    ...
    glPushMatrix();
    glTranslatef(_carPositionX, 0.0, 0.0); // Move the car horizontally
    drawTruck();
    glPopMatrix();
    ...
    glPushMatrix();
    glTranslatef(_carPositionX + 5.54, 0.0, 0.0); // Move the car horizontally
    glRotatef(_angle1, 0.0f, 0.0f, 1.0f);
    drawWheel();
    glPopMatrix();
...
void update(int value) {
    ...
    _carPositionX += 0.05f; // Adjust the speed of the car

    if (_carPositionX > 12.0f) {
        _carPositionX = -13.0f; // Reset the car position when it goes out of the window
    }
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Car with moving left to right");
